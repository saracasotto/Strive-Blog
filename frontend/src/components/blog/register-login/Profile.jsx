...
const Profile = () => {
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const result = await response.json();
        setUserData({ ...userData, avatar: result.avatar });  // Aggiorna l'avatar nello stato
      } else {
  ...
  return (
    <Container className="mt-5">
      ...
      <h3>Cambia Avatar</h3>
      {userData.avatar && (
        <Image src={`http://localhost:5000/authors/${userData.avatar}`} roundedCircle className="mb-3" width={150} />
      )}
